...
namespace Meshwright
{
    ...
    /// every spot Meshwright wrote claimed to be neither, which is a positive assertion rather than an
    ...
    public static class SniperSpotClassifier
    {
        ...
        public static Result Classify(NavFile nav, BspVisibility vis, NavProgress? progress = null)
        {
            ...
            // Chunks of one, dynamically handed out, rather than Parallel.For's default range split.
            //
            // The cost of a spot varies by orders of magnitude - one overlooking a courtyard traces
            // thousands of samples, one in a cupboard is culled to nothing - and there are only a few
            // hundred spots. A static range split therefore leaves some threads with all the expensive
            // ones: measured on gm_construct it scaled 4.95x on sixteen threads, where handing work out
            // one spot at a time gets far closer to the machine. The per-chunk overhead is irrelevant
            // next to a spot that casts a hundred thousand rays.
            Parallel.ForEach(Partitioner.Create(0, work.Count, 1), NavConcurrency.Options,
                () => new Result(), (range, _, local) =>
            {
            for (int w = range.Item1; w < range.Item2; w++)
            {
